...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  $$result.css.add(css);
  return `${$$result.head += `<!-- HEAD_svelte-ylbs26_START -->${$$result.title = `<title>${escape(data.meta.title)}</title>`, ""}<meta property="og:type" content="article"><meta property="og:title"${add_attribute("content", data.meta.title, 0)}><!-- HEAD_svelte-ylbs26_END -->`, ""} <article class="svelte-8uncrq"><hgroup><h1 class="svelte-8uncrq">${escape(data.meta.title)}</h1> <p class="svelte-8uncrq">Published at ${escape(formatDate(data.meta.date))}</p></hgroup> <div class="tags svelte-8uncrq">${each(data.meta.categories, (category) => {
