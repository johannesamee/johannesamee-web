...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_page();
  return `${$$result.head += `<!-- HEAD_svelte-fbczdu_START -->${$$result.title = `<title>${escape(title)}</title>`, ""}<!-- HEAD_svelte-fbczdu_END -->`, ""} <section class="grid grid-cols-[1fr_256px] gap-16 max-w-screen-lg"><div class="prose lg:prose-xl prose-headings:mb-1 prose-p:mt-1 col-span-full" data-svelte-h="svelte-18ltxwj"><h1>Notebook</h1> <p>Ideas, notes and whatever else Johannes publish.</p></div> <section class="col-span-1">${each(data.posts, (post) => {
